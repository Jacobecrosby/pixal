...
def run_detection(dataset, model_path, metric_dir, one_hot_encoding, config=None):
    ...
    if config.plotting.plot_anomaly_heatmap:
        pltm.plot_mse_heatmap_overlay(
            ...
            threshold=loss_config.plotting.loss_cut, use_log_threshold=loss_config.plotting.use_log_loss
        )
    ...
    if config.plotting.plot_loss:
        pltm.plot_pixel_loss_and_log_loss(X_test, predictions, metric_dir, loss_threshold=loss_config.plotting.loss_cut)
        pltm.plot_channelwise_pixel_loss(X_test, predictions, config, metric_dir, loss_threshold=loss_config.plotting.loss_cut)
